...
// Fetch ratings, food repetition, and price data
$query = "
    SELECT
        d.d_id,
        d.title AS food_name,
        COUNT(r.rating_id) AS repetition,
        AVG(r.rating) AS avg_rating,
        AVG(d.price) AS avg_price
    FROM dishes d
    LEFT JOIN ratings r ON d.d_id = r.d_id
    GROUP BY d.d_id
";

$algo_result = $db->query($query);

$foods = [];
if ($algo_result->num_rows > 0) {
    while ($row = $algo_result->fetch_assoc()) {
        $foods[] = $row;
    }
}

// Extract and normalize values
$repetitions = array_column($foods, 'repetition');
$ratings = array_column($foods, 'avg_rating');
$prices = array_column($foods, 'avg_price');

$norm_repetitions = normalize($repetitions);
...
foreach ($foods as $index => $food) {
    $score = $norm_repetitions[$index] * 0.5 + $norm_ratings[$index] * 0.4 - $norm_prices[$index] * 0.1;
    $best_foods[] = [
...
<body class="home">
    ...
    <!-- Hero Section -->
    <section class="hero bg-image" data-image-src="images/img/bag2.jpg">
    ...
    <section class="popular">
        <div class="container">
            <div class="title text-xs-center m-b-30">
                <h2>Best Dishes of the Month</h2>
            </div>
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php foreach ($best_foods as $rank => $food) : ?>
                            <tr>
                                <td><?php echo $rank + 1; ?></td>
                                <td><?php echo htmlspecialchars($food['food_name']); ?></td>
                                <td><?php echo htmlspecialchars($food['score']); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
